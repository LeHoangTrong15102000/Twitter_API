...
class TweetsService {
  // Viết các chức năng của tweets collection vào đây
  ...
  async createTweet(user_id: string, body: TweetRequestBody) {
    ...
    // Sau khi tạo xong tweet thì tìm kiếm và trả về luôn cho client
    const tweet = await databaseService.tweets.findOne({ _id: result.insertedId })
    return tweet
  }

  async increaseView(tweet_id: string, user_id?: string) {
    const inc = user_id ? { user_views: 1 } : { guest_vỉews: 1 }
